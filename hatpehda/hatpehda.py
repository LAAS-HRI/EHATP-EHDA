...
import hatpehda.CommonModule as CM
import hatpehda.NodeModule as NM
...
def heuristic_exploration(n_plot):
    ...
    if CM.g_compute_gui:
        n_plot += 1
        hatpehda.gui.show_tree(first_node, str(n_plot), view=CM.g_view_gui)
        hatpehda.gui.show_all(NM.get_last_nodes_action(first_node), CM.g_robot_name, CM.g_human_name, 0, CM.g_with_contrib, with_begin="false", with_abstract="false")
    if CM.g_stop_input:
        input()
    ...
    while len(e_flagged_nodes)>0:
        ...
        if CM.g_compute_gui:
            n_plot+=1
            hatpehda.gui.show_tree(first_node, str(n_plot), view=CM.g_view_gui)
            hatpehda.gui.show_all(NM.get_last_nodes_action(first_node), CM.g_robot_name, CM.g_human_name, 0, CM.g_with_contrib, with_begin="false", with_abstract="true")
        if CM.g_stop_input:
            input()
    ...
    while True:
        ...
        if rel_divs!=None and CM.g_with_delay:
            if len(rel_divs.divergences)==1 and newagents.get_fluent_obs(rel_divs.divergences[0].property)==ObsType.INF:
                node = create_delay_branch(node, rel_divs, newagents, previous_action, current_cost, e_flagged_nodes, new_e_flagged_nodes, u_flagged_nodes, "e")
        if kill_branch:
...
def refine_u_nodes(first_node, u_flagged_nodes, n_plot):
    while u_flagged_nodes!=[]:
        ...
        if CM.g_compute_gui:
            n_plot+=1
            hatpehda.gui.show_tree(first_node, str(n_plot), view=CM.g_view_gui)
            hatpehda.gui.show_all(NM.get_last_nodes_action(first_node), CM.g_robot_name, CM.g_human_name, with_begin="false", with_abstract="true")
        if CM.g_stop_input:
            input()
...
def create_delay_branch(node, rel_divs, newagents, previous_action, current_cost, e_flagged_nodes, new_e_flagged_nodes, u_flagged_nodes, phase):
    ...
    if div.in_agents(initial_agents):
        ...
        while not found:
            print("\t\ttesting n=", n)
            if not div.in_agents(n.start_agents) and div.in_agents(n.end_agents):
                found=True
            else:
        ...
        find_agents = deepcopy(div_node.start_agents)
        ...
        while not div_action_found:
            ...
            if action.name == "COM_ALIGN":
                prop, key, val = action.parameters[0].split("-")
                print(prop, key, val)
