...
def o_get_ready_precond(state, agent):
    return ( agent == "H"
            and state.agent_at[agent] == "pantry" 
            and not state.agent_ready[agent])

def o_get_ready_effects(state, agent):
    state.agent_ready[agent] = True

o_get_ready = CM.Operator("get_ready", pre_cond=o_get_ready_precond, effects=o_get_ready_effects)


def o_get_ingredient_precond(state, agent):
    return ( agent == "H"
            and state.agent_at[agent] == "pantry" 
            and state.agent_ready[agent] == True
...
human_ops = common_ops + [o_get_ready, o_move, o_get_ingredient, o_put_ingredient, o_aux_done_cooking]
...
def m_Prepare_Dinner_multi_decomp_h1(state, agent):
    multi_subtasks = []
    if state.agent_at[agent] == "kitchen" and state.ingredient_at["ingredient"] == "pantry":
        multi_subtasks.append([("Bring_Ingredient_From_Pantry", ), ("Put_Ingredient", ), ("Done_Cooking", ), ("Prepare_Dinner", )])
    ...
    return multi_subtasks    
m_Prepare_Dinner_h1 = CM.Method("Prepare_Dinner", pre_cond=m_Prepare_Dinner_precond, done_cond=m_Prepare_Dinner_donecond, multi_decomp=m_Prepare_Dinner_multi_decomp_h1)
...
def m_Bring_Ingredient_From_Pantry_multi_decomp(state, agent):
    ...
    if state.agent_at[agent] == "kitchen":
        multi_subtasks.append([("move", "kitchen", "pantry"), ("get_ready", ), ("get_ingredient", ), ("move", "pantry", "kitchen")])
    return multi_subtasks
...
def m_Prepare_Dinner_precond_r1(state, agent):
    return state.agent_at[agent] == "kitchen" and (not state.cut["vegetable"] or not state.washed["vegetable"])

def m_Prepare_Dinner_multi_decomp_r1(state, agent):
    multi_subtasks = []
    if state.agent_at[agent] == "kitchen" and (not state.cut["vegetable"] or not state.washed["vegetable"]):
        multi_subtasks.append([("Cut_n_Wash",), ("Prepare_Dinner",)])    
    return multi_subtasks
m_Prepare_Dinner_r1 = CM.Method("Prepare_Dinner", pre_cond=m_Prepare_Dinner_precond_r1, done_cond=m_Prepare_Dinner_donecond, multi_decomp=m_Prepare_Dinner_multi_decomp_r1)

###
def m_Prepare_Dinner_precond_r2(state, agent):
    return (state.agent_at[agent] == "kitchen" and state.washed["vegetable"]
            and not state.seasoned["vegetable"] and not state.boiling["vegetable"])

def m_Prepare_Dinner_multi_decomp_r2(state, agent):
    multi_subtasks = []    
    if state.agent_at[agent] == "kitchen" and state.washed["vegetable"]:
        multi_subtasks.append([("seasoning", ), ("put_on_stove", ), ("Prepare_Dinner", )])
    # elif state.agent_at[agent] == "kitchen" and state.washed["vegetable"] and state.seasoned["vegetable"]:
    #     multi_subtasks.append([("put_on_stove", ), ("Prepare_Dinner", )])
    return multi_subtasks
m_Prepare_Dinner_r2 = CM.Method("Prepare_Dinner", pre_cond=m_Prepare_Dinner_precond_r2, done_cond=m_Prepare_Dinner_donecond, multi_decomp=m_Prepare_Dinner_multi_decomp_r2)

def m_Prepare_Dinner_multi_decomp_r3(state, agent):
    multi_subtasks = []    
    if state.agent_at[agent] == "kitchen" and state.washed["vegetable"]:
        multi_subtasks.append([("put_on_stove",), ("seasoning",), ("Prepare_Dinner",)])
    # elif state.agent_at[agent] == "kitchen" and state.washed["vegetable"] and state.boiling["vegetable"]:
    #     multi_subtasks.append([("seasoning",), ("Prepare_Dinner",)])
    ##        
    return multi_subtasks
m_Prepare_Dinner_r3 = CM.Method("Prepare_Dinner", pre_cond=m_Prepare_Dinner_precond_r2, done_cond=m_Prepare_Dinner_donecond, multi_decomp=m_Prepare_Dinner_multi_decomp_r3)
...
robot_methods = common_methods + [m_Prepare_Dinner_r1, m_Prepare_Dinner_r2, m_Prepare_Dinner_r3]
human_methods = common_methods + [m_Prepare_Dinner_h1, m_Done_Cooking_h3, m_Bring_Ingredient_From_Pantry_h1, m_Put_Ingredient_h1]
...
def initDomain():
    ...
    initial_state.create_dyn_fluent("agent_ready", { 
        "H" : False,
        "R" : False
    })
