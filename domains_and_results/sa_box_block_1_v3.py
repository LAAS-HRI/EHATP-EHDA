            ...
            if cube_reachable_by(state, cube_name, agent):
                for box in state.box_at_table:
                    if(cube_can_be_placed_1(state, cube_name, box)):
                        multi_subtasks.append([ ("pick", cube_name), ("Place_1_near", cube_name, box), ("Pick_n_place",) ])
            else: # Unstack
                for box in state.box_at_table:
                    if(cube_can_be_placed_1(state, cube_name, box)):
                        multi_subtasks.append([ ("Pick_1", cube_name), ("Place_1_near", cube_name, box), ("Pick_n_place",) ])
...
def m_Place_in_box_precond(state, agent, cube_name, box):
...
def m_Place_in_box_multi_decomp(state, agent, cube_name, box):
    multi_subtasks = []
    ...
    return multi_subtasks
m_Place_in_box = CM.Method("Place_1_near", pre_cond=m_Place_in_box_precond, multi_decomp=m_Place_in_box_multi_decomp, get_precond=m_Place_in_box_precond)
############
# 
# 
# 
...
# purpose is to connect the two kinds of places
def m_Place_1_connect_precond(state, agent, cube_name):
    flag_in_context = state.holding[agent]==cube_name and state.agent_at[agent]["agent_at_table"] == "table"
...
def m_Place_1_connect_multi_decomp(state, agent, cube_name):
    multi_subtasks = []

    if state.agent_in_context[agent]["table_context"] == "table":
        for box in state.box_at_table:
            if(cube_can_be_placed_1(state, cube_name, box)):
                multi_subtasks.append([("Place_1_near", cube_name, box)])
    

    return multi_subtasks
m_Place_1_connect = CM.Method("Place_1_connect", pre_cond=m_Place_1_connect_precond, multi_decomp=m_Place_1_connect_multi_decomp)
...
def m_Place_1_far_multi_decomp(state, agent, cube_name):
    ...
    if state.agent_in_context[agent]["table_context"] == "table":
        multi_subtasks.append([("move_to_table", state.agent_at[agent]["agent_at_table"], "table"), ("Place_1_connect", cube_name)])
    else:
        multi_subtasks.append([ ("change_table_context", state.agent_at[agent]["agent_at_table"], "table"), 
                    ("move_to_table", state.agent_at[agent]["agent_at_table"], "table"), ("Place_1_connect", cube_name)])
...
robot_methods = common_methods + []
human_methods = common_methods + [m_Pick_1_far, m_Place_1_far, m_Place_1_connect, m_Pick_n_place_far]
