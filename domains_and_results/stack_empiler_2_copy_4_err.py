...
def applyRulesForSituationAssessment(state, w_the_possible_state_to_keep):
    ...
    if state.agent_in_context["H"]["table_context"] == "table" and state.agent_at["H"]["agent_at_table"] == "table":
        ...
        # currently let us focus on at_table(cube) fact only:
        for cube in state.color_cubes:
            if (state.at_table[cube]["at_table"] != 
                w_the_possible_state_to_keep.at_table[cube]["at_table"]):
                return None
        
        # currently let us focus on "cube on something" fact only:
        for cube in state.color_cubes:
            if (state.color_cubes[cube]["on"] != 
                w_the_possible_state_to_keep.color_cubes[cube]["on"]):
                return None
            
        # currently let us focus on inspected cube fact only:
        ...
        # if a part of the solution was already achieved:
        for cube in state.color_cubes:
            if (state.solution[cube]["on"] != 
                w_the_possible_state_to_keep.solution[cube]["on"]):
                return None
...
def o_communicate_precond(state, agent_s, agent_r, cube_name):
    if (state.agent_at[agent_s]["agent_at_table"] == "table" and state.agent_at[agent_r]["agent_at_table"] == "table"):
        ...
        return False
def o_communicate_effects(state, agent, cube_name, side):
    return True
o_communicate = CM.Operator("communicate-inspected-status-of", pre_cond=o_communicate_precond, effects=o_communicate_effects)
...
def m_Communicate_precond(state, agent_s, agent_r, cube_name):
    if (state.agent_at[agent_s]["agent_at_table"] == "table" and state.agent_at[agent_r]["agent_at_table"] == "table"):
        if state.inspected_cube_to_place[cube_name]["inspected"] == True:
            return True
        else:
...
def m_Communicate_decomp(state, agent_s, agent_r, cube_name):
    multi_subtasks = []
    multi_subtasks.append(("communicate-inspected-status-of", cube_name))    
...
## Place -- capturing communication subtask ##
