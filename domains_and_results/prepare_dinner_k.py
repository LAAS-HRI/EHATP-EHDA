...
## operator - change focus towards
def o_change_focus_precond(state, agent, place_i, place_j):
    return ( agent == "H" 
            and state.agent_in_context[agent] == place_i 
            and state.agent_at[agent] == place_i) 
def o_change_focus_effects(state, agent, place_i, place_j):
    state.agent_in_context[agent] = place_j
o_change_focus = CM.Operator("change_focus_towards", pre_cond=o_change_focus_precond, effects=o_change_focus_effects)

## operator - to move to the other table ##
## context for the agent to the table is required before moving close to it
def o_move_precond(state, agent, place_i, place_j):
    return ( agent == "H" 
            and state.agent_in_context[agent] == place_j 
            and state.agent_at[agent] == place_i) 
...
human_ops = common_ops + [o_change_focus, o_move, o_get_ingredient, o_put_ingredient, o_aux_done_cooking]
...
def m_Bring_Ingredient_From_Pantry_multi_decomp(state, agent):
    multi_subtasks = []
    if (state.agent_at[agent] == "kitchen" and state.agent_in_context[agent] == "kitchen"):
        multi_subtasks.append([("change_focus_towards", "kitchen", "pantry"), 
                               ("move", "kitchen", "pantry"), 
                               ("get_ingredient", ), 
                               ("change_focus_towards", "pantry", "kitchen"),
                               ("move", "pantry", "kitchen")])
    return multi_subtasks
