...
## return a set of state variables whose values can be observed by human in state 
## the idea is to remove all possible states human is uncertain about which Human can marley discard by visualizing the designated world 
def applyRulesForSituationAssessment(state):
    
    state_post_sa = None #appropriate ds?
    
    # E.g., if human is in the context of table and present at the table
    # they will assess all cubes orientation
    # Note that we will specify only a single consequent that is OBSERVABLE w.r.t. an antecedent (i.e. a formula)
    # these rules will be "stratified" - divided into multiple strata
    if state.agent_in_context["H"]["table_context"] == "table" and state.agent_at["H"]["agent_at_table"] == "table":
        for cube_name in state.cube_at_table:
            if state.cube_at_table[cube_name]["at_table"] == "table":
                state_post_sa.cube_at_table[cube_name]["at_table"] = "table"

        for cube_name in state.color_cubes:
            if state.holding["R"] == cube_name:
                state_post_sa.color_cubes[cube_name]["on"] = []
                state_post_sa.cube_at_table[cube_name]["at_table"] = None
            
        for loc in state.solution:
            if state.locations[loc] != None:
                state_post_sa.color_cubes[state.locations[loc]]["on"] = []
                state_post_sa.cube_at_table[state.locations[loc]]["at_table"] = None
                state_post_sa.locations[loc] = state.locations[loc]
    ...
    return state_post_sa     
r_turn = CM.RulesForSA("get_sa_done_in_this_state", rule_antecedent_consequent = applyRulesForSituationAssessment)
...
o_inspect = CM.Operator("inspect_cube", pre_cond=o_inspect_precond, effects=o_inspect_effects)
            ...
            if (state.color_cubes[cube_name]["color"] in state.solution[l]["colors"] 
                ...
                if inspect_target_supports(state, l) == []:
                    ...
                    for cube in inspect_target_supports(state, l):
                        multi_subtasks_loc.append(("inspect_cube", cube))
