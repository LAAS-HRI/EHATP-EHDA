...
"""
## clean cube operator ##
# idea: if the cube is relevant for the stack, an agent picks it and cleans it
def o_clean_donecond(state, agent, cube):
    return (state.prepare_cleaned_base[cube]["cleaned"] == True)
def o_clean_precond(state, agent, cube): 
    return (state.holding[agent] == cube
        and state.prepare_cleaned_base[cube]["cleaned"] == False)     
def o_clean_effects(state, agent, cube):
    state.prepare_cleaned_base[cube]["cleaned"] = True
o_clean_cube = CM.Operator("clean_cube", done_cond=o_clean_donecond, pre_cond=o_clean_precond, effects=o_clean_effects)
"""
...
# "m_get_precond or in other words m_get_precond_for_comm" 
...
def m_get_precond(state, agent, cube_name):    
        ...
        for l in state.solution:
            if (state.color_cubes[cube_name]["color"] in state.solution[l]["colors"] 
                ...
                and target_free(state, l)):
                    # I don't think returning only this inspected list is sufficient 
                    # for POC it is fine
...
def m_Place_multi_decomp(state, agent, cube_name):
    ...
    return multi_subtasks
m_Place = CM.Method("Place", pre_cond=m_Place_precond, multi_decomp=m_Place_multi_decomp, get_precond=m_get_precond)
