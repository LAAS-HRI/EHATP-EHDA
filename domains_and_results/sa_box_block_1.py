...
o_turn = CM.Operator("change_focus_towards", pre_cond=o_turn_precond, effects=o_turn_effects)
...
def o_take_out_precond(state, agent, cube_name, box):
    if (cube_name in state.box_containing[box]["contains"] and state.holding[agent] == None):
        return True   
    else:
        return False
                                
def o_take_out_effects(state, agent, cube_name, box):
    if(len(state.box_containing[box]["contains"]) == 1):
        state.box_at_table[box]["empty"] = True
    state.box_containing[box]["contains"].remove(cube_name)    
    state.holding[agent] = cube_name
    

o_take_out = CM.Operator("take_out", pre_cond=o_take_out_precond, effects=o_take_out_effects)
...
def o_get_effect(state, agent, cube, box):        
    return cube_can_be_placed_1(state, cube, box)

def o_communicate_precond(state, agent, cube, box):
    # if(agent != "R"):
    #     return False
    return True

def o_communicate_effects(state, agent, cube, box):
    True == True
...
o_communicate = CM.Operator("communicate_if_cube_can_be_put", pre_cond=o_communicate_precond, effects=o_communicate_effects, get_effect=o_get_effect)
...
common_ops = [o_pick, o_place_1, o_communicate]
robot_ops = common_ops + [o_take_out]
human_ops = common_ops + [o_turn, o_move]
...
## Communicate ##
def m_Communicate_precond(state, agent, cube, box):    
    return True
    
def m_Communicate_decomp(state, agent, cube, box):
    multi_subtasks = []
    # multi_subtasks.append([("communicate_inspected_status_of_cube", cube_name)])    
    multi_subtasks.append([("communicate_if_cube_can_be_put", cube, box)])    
...
## Pick_n_place_near ## new and pick and place from near ##
def m_Pick_n_place_donecond_ll(state, agent):
    return goal_reached_1(state)
def m_Pick_n_place_precond_ll(state, agent):
    if(state.holding[agent] != None):
            return True
    return False    

def m_Pick_n_place_multi_decomp_ll(state, agent):
    multi_subtasks = []
    for cube_name in state.color_cubes:
        if state.holding[agent] == cube_name:
            multi_subtasks.append([ ("Place_1", cube_name), ("Pick_n_place",) ])
    return multi_subtasks
m_Pick_n_place_ll = CM.Method("Pick_n_place", pre_cond=m_Pick_n_place_precond_ll, done_cond=m_Pick_n_place_donecond_ll, multi_decomp=m_Pick_n_place_multi_decomp_ll)
...
def m_Pick_n_place_multi_decomp(state, agent):
    ...
    for cube_name in state.color_cubes:
        if cube_side_reachable_by(state, cube_name, agent):
            ...
            # commented later also removed Pick far and place far old version
            # else: # Unstack
...
def m_Pick_1_far_multi_decomp(state, agent, cube_name):
    ...
    if state.agent_in_context[agent]["table_context"] == state.cube_at_table[cube_name]["at_table"]:
        ...
        multi_subtasks.append([ ("change_focus_towards", state.agent_at[agent]["agent_at_table"], state.cube_at_table[cube_name]["at_table"]), 
...
# this is specially required
def m_Place_1_primitive_in_box_get_precond(state, agent, cube_name, box):   

    return [[cube_name, box]]

def m_Place_1_primitive_in_box_precond(state, agent, cube_name, box):
    flag_in_context = state.holding[agent]==cube_name and state.agent_at[agent]["agent_at_table"] == "table"
       
    # why do we need this?
    if flag_in_context and cube_can_be_placed_1(state, cube_name, box):        
        return True
    return False

def m_Place_1_primitive_in_box_multi_decomp(state, agent, cube_name, box):
    multi_subtasks = []
      
    multi_subtasks.append([ ("place_1", cube_name, box) ])                     

    return multi_subtasks
m_Place_1_primitive_in_box = CM.Method("Place_1_primitive", pre_cond=m_Place_1_primitive_in_box_precond, multi_decomp=m_Place_1_primitive_in_box_multi_decomp, get_precond=m_Place_1_primitive_in_box_get_precond)
############
...
def m_Place_in_box_multi_decomp(state, agent, cube_name):
    ...
    for b in state.box_at_table:
        if cube_can_be_placed_1(state, cube_name, b):
            multi_subtasks.append([ ("Place_1_primitive", cube_name, b) ])                 
...
def m_Place_1_far_multi_decomp(state, agent, cube_name):
    ...
    if state.agent_in_context[agent]["table_context"] == "table":
        ...
        multi_subtasks.append([ ("change_focus_towards", state.agent_at[agent]["agent_at_table"], "table"), 
...
# m_Pick_n_place_ll,
common_methods = [m_Communicate, m_Pick_n_place, m_Place_in_box, m_Place_1_primitive_in_box]
robot_methods = common_methods + []
...
def initDomain():
    ...
    initial_state.create_static_fluent("box_transparent_type", {         
        "box_1" : {"type": "opaque"}, # opaque or transparent
        "box_2" : {"type": "opaque"}  # opaque or transparent
    })
